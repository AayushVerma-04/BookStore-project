  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-500 to-purple-600 text-white">
      <form className="bg-white/20 backdrop-blur-md shadow-xl rounded-2xl p-8 w-full max-w-md">
        ...
        <div className="mb-6">
          <label htmlFor="pass" className="block text-white text-sm mb-1">Password</label>
          <input
            type="password"
            name="pass"
            id="pass"
        ...
        <button 
          disabled={loading} 
          className="w-full bg-white text-blue-600 font-semibold py-2 rounded-xl shadow-lg hover:scale-105 hover:bg-blue-200 transition-all duration-300"
          onClick={handleLogin}
        >
          Login
        </button>

        {error && <div className="text-center text-red-300 mt-4">{error}</div>}
      </form>
    </div>
  );
};
